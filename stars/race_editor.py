...
__defaults = {
    ...
    'race_editor_energy_per_kt': [5, 1, 100],
...
class RaceEditor(Defaults):
    ...
    def post(self, action):
        ...
        c = ((self.race_editor_effort_per_kt/1000)-1)*1000 + ((((self.race_editor_effort_per_kt/1000)-1)*10)+1) * self.race_editor_effort_per_kt/100
        if self.race_editor_effort_per_kt > 1000:
            c /= 2
        ap -= c
        c = round(log(self.race_editor_energy_per_kt/10, 2)*1000)
        if self.race_editor_energy_per_kt > 10:
            c /= 2
        ap -= c+500
